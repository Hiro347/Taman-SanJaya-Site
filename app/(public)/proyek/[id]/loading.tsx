...
export default function ProjectDetailLoading() {
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-16 animate-pulse">
      ...
      <div className="relative w-full aspect-[16/10] sm:aspect-[21/10] md:max-h-[520px] rounded-xl sm:rounded-2xl overflow-hidden bg-brand-earth/15 border border-brand-earth/20 shadow-md flex items-center justify-center">
      ...
      <div className="mt-8 sm:mt-10 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="h-10 sm:h-12 w-3/4 bg-brand-earth/15 rounded-xl" />
          ...
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div className="h-12 bg-white/60 rounded-lg border border-brand-earth/10" />
            <div className="h-12 bg-white/60 rounded-lg border border-brand-earth/10" />
          </div>
        </div>

        {/* Kolom Kanan: Lokasi */}
        <div className="lg:col-span-4 bg-white/80 rounded-xl sm:rounded-2xl p-6 sm:p-7 border border-brand-earth/15 shadow-sm space-y-2">
      ...
      <div className="mt-14 sm:mt-18 pt-8 border-t border-brand-earth/15">
        <div className="h-8 w-60 bg-brand-earth/15 rounded-lg mb-6" />
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className="aspect-[16/11] rounded-lg sm:rounded-xl bg-white/60 border border-brand-earth/15"
            />
          ))}
        </div>
      </div>

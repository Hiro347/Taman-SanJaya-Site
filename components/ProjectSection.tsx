        ...
        {projects.map((project, index) => {
          ...
          return (
            <Link
              ...
              className={`block focus:outline-hidden ${isFeatured ? 'md:col-span-2' : ''}`}
            >
              <motion.div
                whileHover={{ y: -6 }}
                transition={{ duration: 0.25, ease: 'easeOut' }}
                className="group bg-white rounded-2xl sm:rounded-3xl overflow-hidden border border-brand-earth/15 shadow-sm hover:shadow-2xl transition-shadow duration-300 flex flex-col justify-between cursor-pointer h-full"
              >
